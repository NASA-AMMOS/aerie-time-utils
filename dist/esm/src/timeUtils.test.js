...
import { convertDoyToYmd, convertDurationStringToPGInterval, convertDurationStringToUs, convertUsToDurationString, pgUTCToMs, getBalancedDuration, getDaysInMonth, getDaysInYear, getDoy, getDoyTime, getDoyTimeComponents, getDurationTimeComponents, getShortISOForDate, getTimeAgo, getUnixEpochTime, isTimeBalanced, isTimeMax, parseDoyOrYmdTime, parseDurationString, removeDateStringMilliseconds, validateTime, } from './timeUtils.js';
...
test('convertDurationStringToInterval', () => {
    expect(convertDurationStringToPGInterval('2y 318d 6h 16m 19s 200ms 0us')).toEqual('2 years 318 days 6 hours 16 minutes 19 seconds 200 milliseconds');
...
test('getTimeAgo', () => {
    ...
    expect(getTimeAgo(new Date(time.getTime() - 1000 * 60 * 60 * 24 * 366), time, 1000 * 60 * 60 * 24 * 366)).toEqual('1y ago');
});
...
test('getDurationTimeComponents', () => {
    expect(getDurationTimeComponents({
        days: 3,
        hours: 10,
        isNegative: false,
        microseconds: 0,
