...
function isTimeMax(time, type) {
    ...
    switch (type) {
        case time_js_2.TimeTypes.ABSOLUTE: {
            const year = (_a = parseDoyOrYmdTime(getDoyTime(new Date(getUnixEpochTime(time))))) === null || _a === void 0 ? void 0 : _a.year;
        ...
        case time_js_2.TimeTypes.RELATIVE: {
            ...
            const year = (_b = parseDoyOrYmdTime(getDoyTime(new Date(getUnixEpochTime(convertDurationToDoy(duration)))))) === null || _b === void 0 ? void 0 : _b.year;
...
function parseDoyOrYmdTime(dateString, numDecimals = 6) {
    const matches = (dateString !== null && dateString !== void 0 ? dateString : '').match(new RegExp(`^(?<year>\\d{4})-(?:(?<month>(?:[0]?[0-9])|(?:[1][0-2]))-(?<day>(?:[0-2]?[0-9])|(?:[3][0-1]))|(?<doy>\\d{1,3}))(?:T(?<time>(?<hour>[0-9]|[0-2][0-9])(?::(?<min>[0-9]|(?:[0-5][0-9])))?(?::(?<sec>[0-9]|(?:[0-5][0-9]))(?<dec>\\.\\d{1,${numDecimals}})?)?)?)?$`, 'i'));
    if (matches) {
...
function getTimeZoneName() {
    ...
    const parts = formatter === null || formatter === void 0 ? void 0 : formatter.formatToParts(Date.now());
