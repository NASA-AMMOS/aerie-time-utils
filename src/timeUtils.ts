...
import { ISO_ORDINAL_TIME_REGEX, SIGNED_SECOND_TIME_REGEX, SIGNED_DOY_TIME_REGEX, SECOND_TIME_REGEX, DOY_TIME_REGEX, ISO_8601_UTC_REGEX, MAX_UTC_YEAR } from './constants/time.js';
...
export function validateTime(time: string, type: TimeTypes): boolean {
  switch (type) {
    ...
    case TimeTypes.SIGNED_DOY_TIME:
      return SIGNED_DOY_TIME_REGEX.exec(time) !== null;
    case TimeTypes.DOY_TIME:
      return DOY_TIME_REGEX.exec(time) !== null;
    case TimeTypes.SIGNED_SECOND_TIME:
      return SIGNED_SECOND_TIME_REGEX.exec(time) !== null;
 ...
 * isTimeMax('-365T23:59:60.999', TimeTypes.SIGNED_DOY_TIME)) // true
 ...
 * 
 * @example
 * convertDurationStringToPostgresInterval('8d 6h 16m') // returns '8 days 6 hours 16 minutes'
 * convertDurationStringToPostgresInterval('30') // returns '30 microseconds'
 * convertDurationStringToPostgresInterval('001T00:01:00') // returns '1 day 1 minute'
 */
export function convertDurationStringToPostgresInterval(duration: string): string | never {
  const parsedDuration = parseDurationString(convertUsToDurationString(convertDurationStringToUs(duration)));
 ...
 *
 * @example
 * convertDurationStringToUs('045T10:05:02.001')) // returns 3924302001000;
 * convertDurationStringToUs('30') // returns 30
 ...
 *
 */
export function convertDurationStringToUs(duration: string): number | never {
...
function parseDOYDurationTime(doyTime: string): ParsedDurationString | null {
  const isEpoch = validateTime(doyTime, TimeTypes.SIGNED_DOY_TIME);
  const matches = isEpoch ? SIGNED_DOY_TIME_REGEX.exec(doyTime) : DOY_TIME_REGEX.exec(doyTime);
  if (matches !== null) {
    if (matches) {
