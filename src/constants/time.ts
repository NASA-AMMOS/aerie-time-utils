...
export const DOY_TIME_REGEX =
  /^(?:(?<doy>\d{3})T)?(?<hr>\d{2}):(?<mins>\d{2}):(?<secs>\d{2})(?:\.(?<ms>\d+))?$/;
export const SECOND_TIME_REGEX = /(?<secs>(\d+))(?:\.(?<ms>\d+))?$/;
export const SIGNED_DOY_TIME_REGEX =
  /^((?<sign>[+-]?))(?<doy>(\d{3}))?(T)?(?<hr>(\d{2})):(?<mins>(\d{2})):(?<secs>\d{2})(?:\.(?<ms>\d+))?$/;
export const SIGNED_SECOND_TIME_REGEX =
  /^(?<sign>([+-]?))(?<secs>(\d+))(?:\.(?<ms>\d+))?$/;
