 ...
 * // us (optional): one or more digits for microseconds
 */
export const ISO_ORDINAL_TIME_REGEX =
  /^(?<year>(\d{4}))-(?<doy>(\d{3}))T(?<hr>(\d{2})):(?<mins>(\d{2})):(?<secs>(\d{2}))(?:\.(?<ms>\d{1,3})(?<us>\d{1,3})?)?$/;
...
export const ISO_8601_UTC_REGEX =
  /^(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2})T(?<hr>\d{2}):(?<mins>\d{2}):(?<secs>\d{2})(?:\.(?<ms>\d{1,3})(?<us>\d{1,3})?)?Z$/;
/**
...
export const DOY_TIME_REGEX =
  /^((?<sign>[+-]?))(?<doy>(\d{3}))?(T)?(?<hr>(\d{2})):(?<mins>(\d{2})):(?<secs>(\d{2}))(?:\.(?<ms>\d{1,3})(?<us>\d{1,3})?)?$/;
/**
